...
class OPTelebot:
    def __init__(self) -> None:
        ...
        async def define_topic(call):
            ...
            try:
                ...
                resp, ids_not_found = query.search(ids=self.local_conf["listId"], index_pattern=self.local_conf.get("index"))

                self.local_conf.update({"ids_not_found": ids_not_found}) # Menyimpan id yang tidak ada ke local conf
    ...
    async def __handle_to_many_requests(self, exception: apihelper.ApiException):
        retry_after = exception.result.json().get("parameters", {}).get("retry_after", 5)
        self.logger.info(f"Rate limit exceeded. Retrying in {retry_after} seconds...")
        await asyncio.sleep(retry_after)
    
    def rate_limit(self, handler):
        async def wrapper(message: Message):
            try:
                await handler(message)
            except apihelper.ApiException as err:
                if err.result.status_code == 429:
                    await self.__handle_to_many_requests( exception=err )
                    await handler(message)
                else:
                    raise err
        return wrapper
